...
const BuyerRegister = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/register", { ...formData, role: "buyer" });
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen p-6 pt-16 transition-colors duration-500 ${darkMode ? "bg-[#000c20] text-white" : "bg-white text-black"}`}>
      ...
      <h2 className="text-3xl font-bold mb-8 mt-4">{translations[language].buyerRegistration}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            ...
            <input
              ...
              required
            />
            <p className="text-xs text-white-500 mt-1 ml-3">{language === "ta" ? "( நீங்கள் பயிர்களை பெறும் இடத்தைக் குறிப்பிடவும்! )"
                                                                               : "( Fill the location from where you will get crops! )"}
            </p>
